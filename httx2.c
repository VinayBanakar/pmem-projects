...
//prototypes
void ht_alloc(PMEMobjpool*, TOID(struct hashtable_s)*, uint32_t, size_t, uint64_t);
...
struct root{
	...
	TOID(struct hashtable_s) ht_list[6]; // 6 HTs for now.
...
// If the bucket size passed for a ht is more than previous then it'll auto expand the table
TOID(struct hashtable_s)* init_pool_ht(const char* path, uint64_t ht_id, size_t buck_sz) {
    ...
    //REVIEW: Nested flexible arrays is causing TX_ALLOC to fail -- alas, no time to debug
...
void ht_alloc(PMEMobjpool *pop, TOID(struct hashtable_s)* hashtable, uint32_t seed, size_t bucket_sz, uint64_t ht_id)
{
...
int ht_migrate(TOID(struct hashtable_s) ht1, TOID(struct hashtable_s) ht2){
    ...
    TX_BEGIN(pop){
        ...
        pmemobj_tx_add_range(buckets_ht1.oid, 0, sz); // ht1 undo logged for contingency.
...
int main(int argc, char* argv[]){
    ...
    char* test = "What is the ultimate answer for life, the universe, and everything?\0";
    PMEMoid TESToid;
...
voit perf_test(){
    ...
    TOID(struct hashtable_s)* ht1 = init_pool_ht(path,0, 10);
    ...
    printf("==== Test 4: Create multiple variable size hash tables in single pool ====\n");
    TOID(struct hashtable_s)* ht2 = init_pool_ht(path,1, 10);
    pmemobj_close(pop);
    TOID(struct hashtable_s)* ht3 = init_pool_ht(path,1, 5);
    pmemobj_close(pop); 
